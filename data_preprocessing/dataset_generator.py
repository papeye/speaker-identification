...
class TrainDSGenerator(_DatasetGenerator):
    def generate_train_valid_ds(self, noises):
        ...
        train_ds = _super.paths_and_labels_to_dataset(train_audio_paths, train_labels)
        train_ds = train_ds.shuffle(
            buffer_size=Config.batch_size * 8, seed=Config.shuffle_seed
        ).batch(Config.batch_size)

        valid_ds = _super.paths_and_labels_to_dataset(valid_audio_paths, valid_labels)
        ...
        train_ds = train_ds.map(
            lambda x, y: (_super.audio_to_fft(x), y),
            num_parallel_calls=tf.data.AUTOTUNE,
        )
